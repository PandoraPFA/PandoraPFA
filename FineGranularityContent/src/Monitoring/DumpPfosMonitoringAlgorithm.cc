...
StatusCode DumpPfosMonitoringAlgorithm::Run()
{
    ...
    m_goodTrackEnergy   = 0;
    m_goodPhotonEnergy  = 0;
    m_goodIdedPhotonEnergy  = 0;
    m_goodNeutralEnergy = 0;
    m_goodIdedNeutralEnergy = 0;
    m_badTrackEnergy   = 0;
    m_badPhotonEnergy  = 0;
    m_badNeutralEnergy = 0;
...
DumpPfosMonitoringAlgorithm::~DumpPfosMonitoringAlgorithm()
{
      ...
      const float econfmat[3][3] =
	{
      ...
      if(m_goodPhotonEnergySum)std::cout <<  " Correctly IDed Photon  Energy Fraction : " << m_goodIdedPhotonEnergySum/m_goodPhotonEnergySum << std::endl;
      if(m_goodNeutralEnergySum)std::cout << " Correctly IDed Neutral Energy Fraction : " << m_goodIdedNeutralEnergySum/m_goodNeutralEnergySum << std::endl;
...
StatusCode DumpPfosMonitoringAlgorithm::DumpChargedPfo(const ParticleFlowObject* pPfo)
{
	...
	(fCharged > m_goodFractionCut) ? m_goodTrackEnergy+=clusterEnergy : m_badTrackEnergy+=clusterEnergy;
	    ...
	    const float tTrack = pTrack->GetTimeAtCalorimeter()- tof;           
            ...
            if (pCluster != NULL)
            {
	      FORMATTED_OUTPUT_TRACK(trackId, mcId, pTrack->CanFormPfo(), pTrack->ReachesCalorimeter(), trackEnergy, mcEnergy, clusterEnergy, clusterTime, chi, leaving, fCharged, fPhoton, fNeutral);
...
StatusCode DumpPfosMonitoringAlgorithm::DumpNeutralOrPhotonPfo(const ParticleFlowObject *pPfo, bool isPhotonPfo)
{
    ...
    for (ClusterList::const_iterator clusterIter = clusterList.begin(); clusterIter != clusterList.end(); ++clusterIter)
    {
	    ...
	    if(fPhoton+fNeutral > m_goodFractionCut){
	      ...
	      if(fPhoton > fNeutral)m_goodIdedPhotonEnergy+=clusterEnergy;
	    }
            else
            {
	    ...
	    if(fNeutral+fPhoton > m_goodFractionCut){
	      ...
	      if(fNeutral > fPhoton)m_goodIdedNeutralEnergy+=clusterEnergy;
	    }
            else
            {
...
float DumpPfosMonitoringAlgorithm::ClusterTime(const Cluster* pCluster) const
{
    ...
    if (sumEnergy > 0.f)avTime = sumTimeEnergy/sumEnergy;
...
StatusCode DumpPfosMonitoringAlgorithm::Initialize()
{
    ...
    m_count = 0;
    m_trackRecoAsTrackEnergySum    =0;
    m_photonRecoAsTrackEnergySum   =0;
    m_neutralRecoAsTrackEnergySum  =0;
    m_photonOrNeutralRecoAsTrackEnergySum   = 0;
    m_photonOrNeutralRecoAsTrackEnergySum2  = 0;
    m_trackRecoAsPhotonOrNeutralEnergySum   = 0;
    m_trackRecoAsPhotonOrNeutralEnergySum2  = 0;
    m_confusionCorrelation =0;
    m_trackRecoAsPhotonEnergySum   =0;
    m_photonRecoAsPhotonEnergySum  =0;
    m_neutralRecoAsPhotonEnergySum =0;
    m_trackRecoAsNeutralEnergySum  =0;
    m_photonRecoAsNeutralEnergySum =0;
    m_neutralRecoAsNeutralEnergySum=0;
    m_goodTrackEnergySum   =0;
    m_goodPhotonEnergySum  =0;
    m_goodIdedPhotonEnergySum =0;
    m_goodNeutralEnergySum =0;
    m_goodIdedNeutralEnergySum =0;
    m_badTrackEnergySum    =0;
    m_badPhotonEnergySum   =0;
    m_badNeutralEnergySum  =0;
